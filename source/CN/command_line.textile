h2. Rails 命令行工具和 Rake 任务

Rails 命令行工具提供了以下功能：

* 创建 Rails 应用
* 生成 models ， controllers ， migrations 以及单元测试
* 启动开发服务器
* 使用 shell 测试 models 对象
* 自定义应用

endprologue.

NOTE: 本教程假设你已阅读 "Rails 初上手指南":getting_started.html ，并掌握了 Rails 的基本知识。

WARNING. 本指南基于 Rails 3.0 ，有些代码在 Rails 3.0 之前的版本上无法正常运行。
...
我们一起来用上面提到的命令创建一个简单的 Rails 应用。

h4. +rails new+

创建 Rails 应用的第一件事就是执行 +rails new+ 命令。

WARNING: 请先通过执行 +gem install rails+ 安装 rails gem 。参考 "Rails 3 Release Notes":/3_0_release_notes.html
