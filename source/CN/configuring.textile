...
* +config.action_controller.asset_path+ 接受一个代码块，这个代码块配置去哪找资源文件。更简短的版本是 +config.asset_path+.
...
* +ActiveSupport::Logger.silencer+ 设置为 +false+ 取消代码块中缄默日志的功能。默认为 +true+
...
Rails 有 5 种初始化事件, 这些事件可以被挂上 hook (列出了它们的运行顺序)
...
*+bootstrap_hook+* 运行所有配置了的 +before_initialize+ 代码块。
