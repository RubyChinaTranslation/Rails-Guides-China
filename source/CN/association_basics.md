...
通过使用模型关联，可以简化一些数据查询的操作。比如一个rails程序中包含一个顾客模型`customer`和一个订单模型`order`，
每个顾客可能有多个订单。
...
下面我们为`Customer`和`Order`添加关联模型，有个小技巧是在拥有外键的模型添加`belongs_to`：
...
下面开始学习不同的关联模型，可以参考方法的API了解更详细的内容

The Types of Associations
-------------------------

在Rails中, `association` 将两个模型关联起来，在模型中声明。如果要声明一个模型属于（belongs_to）另一个，需要在这个模型中维护一个外键`Foreign Key`，Rails支持一下6种关联关系:

- `belongs_to`
- `has_one`  一对一关联
- `has_many`  一对多关联
- `has_and_belongs_to_many`  多对多关联，通过中间表
- `has_many :through`  也是通过中间表，不过更灵活，一般建议使用`through`，而不使用`has_and_belongs_to_many`
- `has_one :through`
...
`belongs_to` association sets up a one-to-one connection with another model,
...
NOTE: `belongs_to` 须使用单数形式.
 If you used the pluralized form in the above example for the `customer` association in the `Order` model, you would be told that there was an "uninitialized constant Order::Customers". This is because Rails automatically infers the class name from the association name. If the association name is wrongly pluralized, then the inferred class will be wrongly pluralized too.
