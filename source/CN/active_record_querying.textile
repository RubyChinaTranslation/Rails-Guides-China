...
如果你想在 +Model.find+ 中使用 +LIMIT+ 来限制结果，那你可以直接结合使用 +limit+ 和 +offset+ 方法来指定 +LIMIT+ 。
...
+none+ 方法会返回一个代表没有记录的替代对象。随后的条件表达式会根据这个空对象继续返回空对象。这在某些需要返回一个替代对象而不是空结果时非常有用。
...
Active Record 提供了 +readonly+ 方法来明确指定不允许修改返回的对象。任何尝试修改只读对象行为都会失败，抛出一个 +ActiveRecord::ReadOnlyRecord+ 异常。
