...
如果我们希望定义一个 model 里面所有的查询都会执行的 scope ，那我们可以在 model 里面使用 +default_scope+ 方法。
...
如果想移除 scope ，我们可以用 +unscope+ 方法。当某个特定的查询不需要默认 scope 时，这个尤其有用。

<ruby>
Client.unscoped.all
</ruby>

这个方法移除所有的 scope ，在数据表上执行普通的查询。
...
这个章节在前言中使用 count 方法作为例子，但是这些操作适用于所有子节。
...
你也可以在 relation 上使用很多查找方法来做复杂的计算：
...
想知道具体参数，可以参见父章节： "计算":#calculation 。
...
Active Record 模拟数据库壳并以一个清晰的格式打印。所以，同样的查询在 PostgreSQL 中会像下面这样：
...
预加载在数据库引擎盖下面会触发多个查询，并且某些查询可能需要用到之前一些查询的结果。所有， +explain+ 方法先执行查询，在做查询计划。比如说：
...
如果设置为一个数字，那一个超过这些秒的查询就会自动触发一个 EXPLAIN 并写入日志。在 relation 中，这个阈值是与获取记录的总时间相比较的。所以，一个 relation 就像是一个工作单位，不管它是不是实现预先加载而涉及了多个查询。
...
了解 EXPLAIN 的输出已经超出本篇指导的范围了，以下的链接或许有帮助：
