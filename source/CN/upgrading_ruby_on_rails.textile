...
Rails 3.2 不赞成<tt>vendor/plugins</tt>，Rails 4.0 将完全移除它们。虽然它不是 Rails 3.2 升级必须的一部分，你可以通过提取它们为 gems 来替代任何插件并将它们添加到你的 Gemfile。如果你选择不将它们做成 gems，你可以将它们放进<tt>lib/my_plugin/*</tt>，并添加一个适当的初始化器在 <tt>config/initializers/my_plugin.rb</tt>中。
...
在这个文件中添加以下内容，如果你希望封装参数到一个内嵌 hash。在新的应用程序中这是默认情况。
